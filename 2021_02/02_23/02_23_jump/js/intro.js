;(function ($) { // 맨 아래의 전달인자(jQuery)를 매개변수$ 가 받아서 실행한다.
    ...
    var intro = { //// <<<<  =(이퀄) 가 들어간 함수는 무조건 맨 아래에서 실행 intro.init();
        ...
        familySiteFn: function () {
            // 셀렉트박스 (목록상자=리스트박스)
            // 셀렉트박스(#family)를 클릭(이벤트:onchange)하고 해당 패밀리 사이트로 링크를 이동
            var $family = $('#footer #family')

            // $family.on({});  // 이벤트를 계속사용
            // $family.one({}); // 이벤트를 한번만 사용
            // $family.off({}); // 이벤트를 꺼버림

            $family.on({
                change:function(){
                    // var = variable 약어 변수값을 할당(대입)하겟다 라는 키워드
                    var x = $(this).val();  // val(jQuery) = value(javascript)
                    // console.log(x);

                    location.href = x;  // x로 이동
                }
            });
